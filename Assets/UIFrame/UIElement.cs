...
namespace Wargon.UI {
    ...
    public interface IUIService {
        ...
        static IUIService CreateInstance<T>() where T : IUIService {
            var method = typeof(T).GetMethod("Create");
            if (method is null) {
                throw new MissingMethodException(typeof(T).Name, "Create");
            }
            var instance = (IUIService)typeof(T).GetMethod("Create").Invoke(null, null);
            return instance;
        }
    }
}
