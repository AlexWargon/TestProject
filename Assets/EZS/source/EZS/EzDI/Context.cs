...
namespace Wargon.DI
{
    public class Context
    {
        ...
        private readonly Type[] constructorParametersTypes;
        ...
        public Context(Type type, DependencyContainer di) {
            ...
            if (typeof(MonoBehaviour).IsAssignableFrom(type))
            {
                ...
                if (constructor != null)
                {
                    ...
                    constructorParametersTypes = new Type[constructor.GetParameters().Length];
                    for (var i = 0; i < constructorParametersTypes.Length; i++)
                        constructorParametersTypes[i] = parameters[i].ParameterType;
        ...
        public void Inject<T>(T obj) where T : class
        {
                ...
                switch (fieldsToInject[i].Item3)
                {
                    case DiType.New:
                        if(di.HasSingle(fieldsToInject[i].Item2)) 
                            contextType.GetField(fieldsToInject[i].Item1,BINDING_FLAGS)?.SetValue(obj, di.GetContainer(fieldsToInject[i].Item2).Get());
                    ...
                    case DiType.Global:
                        if(di.HasGlobal((fieldsToInject[i].Item2)))
                            contextType.GetField(fieldsToInject[i].Item1,BINDING_FLAGS)?.SetValue(obj, DependencyContainer.Globals[fieldsToInject[i].Item2]);
                        //else Debug.LogError($"{fieldsToInject[i].Item1} can't be inhected. There no isntance in DI");
                        break;
                }
            ...
            {
                var newParams = new object[constructorParametersTypes.Length];
